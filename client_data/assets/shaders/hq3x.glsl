...
void main()
{
	//gl_Position = gl_ProjectionMatrix * vec4(VertexCoord, 1.0);
	gl_Position = gl_ProjectionMatrix * gl_ModelViewMatrix * vec4(VertexCoord, 1.0);
	...
	vTexCoord[0].zw = ps;
	vTexCoord[0].xy = gl_MultiTexCoord0.xy;
	vTexCoord[1] = gl_MultiTexCoord0.xxxy + vec4(-dx, 0, dx, -dy); //  w1 | w2 | w3
	vTexCoord[2] = gl_MultiTexCoord0.xxxy + vec4(-dx, 0, dx,   0); //  w4 | w5 | w6
	vTexCoord[3] = gl_MultiTexCoord0.xxxy + vec4(-dx, 0, dx,  dy); //  w7 | w8 | w9
}
...
void main()
{
	...
	vec4 p1_rgba = texture2D(Texture, vTexCoord[0].xy).rgba;
	vec3 p1  = p1_rgba.rgb;
